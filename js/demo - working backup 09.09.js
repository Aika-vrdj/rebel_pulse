...
function checkGameCompletion() {
  // Comprobamos si todas las canciones DE LOS PACKS QUE TIENE EL JUGADOR tienen al
  // menos 60% de precisión — las canciones de packs de pago no comprados no cuentan,
  // para que la campaña siga siendo completable sin haber comprado todo el DLC.
  const ownedSongs = SONGS.filter(s => isPackOwned(getPackById(s.packId)));
  const allCompleted = ownedSongs.length > 0 && ownedSongs.every(s => (s.bestAccuracy || 0) >= 60);
  
  if (allCompleted) {
    const alreadyCompleted = localStorage.getItem(COMPLETION_KEY);
    if (!alreadyCompleted) {
